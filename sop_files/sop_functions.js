			...
			// °° ASYNC FUNCTIONS 
				...
				class DOMdata {
				 ...
				 renderTask(TaskCollection, StatesCollection) {
					...
					// Badge // 
					TEMPLATE = TEMPLATE + "<span class='LOP-state'> <button onclick='changestate($(this),-1)' class='controls'>&nbsp;<&nbsp;</button> Site : <span data-id=%STATEID% class='badge badge-warning' style='background-color:%THEME%'>%STATENAME%</span> <button onclick='changestate($(this),+1)' class='controls'>&nbsp;>&nbsp;</buton></span>";
					// UDPATE 
					...
					TaskCollection.forEach((element,idx) => {
						...
						html = html + TEMPLATE
							...
							.replace("%STATENAME%", StateCollection[element.stateid].label)
							.replace("%THEME%", StateCollection[element.stateid].theme)
							.replace("%MARKER%", StateCollection[element.stateid].theme);
					} );
